...
public class SolidColorBrushToColorOpacityConverter : IValueConverter
{
    /// <inheritdoc />
    public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        {
            return Math.Round(Colors.White.A / 256d, 2);
        }
        else if (value is SolidColorBrush solidColorBrushResult)
        {
            return Math.Round(solidColorBrushResult.Color.A / 256d, 2);
        }
        else if(value is Brush brushResult)
        {
            var solidBrush = brushResult as SolidColorBrush;
            return Math.Round(solidBrush.Color.A / 256d, 2);
        }
